...
from cfg import SYSTEM_LABELS, JUNK_LABELS, SYNC_PAGE_SIZE, DEFAULT_TARGET_LABELS
from data_store import (
    init_db, upsert_message, mark_review, fetch_for_training, 
    fetch_for_prediction, save_prediction, Message
)
from gmail_client import (
    list_messages, get_message, modify_labels, trash_message, 
    ensure_label, get_labels
)
                ...
                try:
                    if a["action"] == "trash" and a["spam_score"] >= 0.85:
                        if not dry_run:
                            trash_message(a["id"])
                            applied += 1
                        self.logger.debug(f"{'DRY: ' if dry_run else ''}Trashed {a['id']} (spam={a['spam_score']:.2f})")
                        ...
                        if not dry_run:
                            ...
                            applied += 1
                        self.logger.debug(f"{'DRY: ' if dry_run else ''}Routed {a['id']} -> {a['target']} (conf={a['conf']:.2f})")
