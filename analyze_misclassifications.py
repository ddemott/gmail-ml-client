    ...
    try:
        ...
        if misclassifications:
            ...
            for category, items in sorted(by_category.items()):
                ...
                if len(items) > 3:
                    print(f"      ... and {len(items) - 3} more misclassified emails in this category")
                    print()
        ...
        if accuracy < 95:
            ...
            top_error_categories = sorted(category_errors.items(), key=lambda x: x[1], reverse=True)[:3]
            
            print(f"• Add more training data for: {', '.join([cat for cat, _ in top_error_categories])}")
...
def show_category_performance():
    ...
    try:
        # Get all categories
        categories = session.query(Message.gold_label).filter(
            Message.reviewed == True,
            Message.gold_label.isnot(None)
        ).distinct().all()
        ...
        for (category,) in categories:
            messages = session.query(Message).filter(
                ...
                Message.reviewed == True,
                Message.text.isnot(None)
            ).limit(20).all()  # Test up to 20 per category
