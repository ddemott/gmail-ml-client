...
class EnhancedGmailService:
    ...
    def get_labels(self, label_id: str | None = None) -> ServiceResponse:
        ...
        try:
            ...
            if label_id:
                ...
                return ServiceResponse.success_response(
                    data=result, message=f"Retrieved label: {label_id}"
                )
            else:
                # Get all labels
                ...
                return ServiceResponse.success_response(
                    data=user_labels, message=f"Retrieved {len(user_labels)} user labels"
                )
    ...
    def sync_emails(self, max_results: int | None = None) -> ServiceResponse:
        ...
        try:
            ...
            if not emails:
                return ServiceResponse.success_response(data=[], message="No new emails to sync")
            ...
            response = ServiceResponse.success_response(
                data=validated_emails, message=f"Synced {len(validated_emails)} emails"
            )
        ...
        try:
            ...
            if cached_prediction:
                ...
                validation_result = validate_prediction(
                    cached_prediction, set(self.config.labels.default_target_labels)
                )

                if validation_result.is_valid:
                    return ServiceResponse.success_response(
                        data=cached_prediction, message="Retrieved cached prediction"
                    )
                else:
            ...
            if not email_validation.is_valid:
                return ServiceResponse.error_response("Invalid email data", email_validation.errors)
            ...
            prediction_validation = validate_prediction(
                prediction_data, set(self.config.labels.default_target_labels)
            )
    ...
    def train_model(self, force_retrain: bool = False) -> ServiceResponse:
        ...
        try:
            ...
            if not validation_result.is_valid:
                return ServiceResponse.error_response(
                    "Training data validation failed", validation_result.errors
                )
    ...
    def apply_label(self, email_id: str, label_name: str) -> ServiceResponse:
        ...
        try:
            ...
            if not label_validation.is_valid:
                return ServiceResponse.error_response("Invalid label", label_validation.errors)
...
class EnhancedEmailSyncService:
    ...
    def full_sync_and_predict(self, max_emails: int | None = None) -> ServiceResponse:
        ...
        try:
            ...
            if not emails:
                return ServiceResponse.success_response(
                    data={"synced": 0, "predicted": 0}, message="No emails to process"
                )
