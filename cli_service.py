...
@app.command()
def train(
    epochs: int = typer.Option(6, "--epochs", "-e", help="Number of training epochs")
):
...
@app.command()
def predict(
    limit: int = typer.Option(50, "--limit", "-l", help="Maximum predictions to show")
):
    ...
    try:
        ...
        for action in actions:
            ...
            spam_color = "red" if action.spam_score > 0.8 else "green" if action.spam_score < 0.3 else "yellow"
            spam_score = f"[{spam_color}]{action.spam_score:.2f}[/{spam_color}]"
            
            # Color code confidence
            conf_color = "green" if action.confidence > 0.9 else "yellow" if action.confidence > 0.7 else "red"
...
@app.command()
def review(
    limit: int = typer.Option(30, "--limit", "-l", help="Maximum messages to review")
):
    ...
    try:
        ...
        print("[yellow]Enter label (SPAM, Work, Personal, etc.). Leave blank to skip. 'q' to quit.[/yellow]")
        ...
        for action in actions:
            ...
            print(f"[bold]Spam Score:[/bold] {action.spam_score:.2f}, [bold]Confidence:[/bold] {action.confidence:.2f}")
            
            # Get user input
            label = console.input("\n[bold green]Your label [ENTER=skip, q=quit]:[/bold green] ").strip()
...
@app.command()
def apply(
    dry_run: bool = typer.Option(True, "--dry-run/--no-dry-run", help="Preview actions without applying"),
    ...
    try:
        ...
        with console.status(f"[bold {'yellow' if dry_run else 'red'}]{'Previewing' if dry_run else 'Applying'} actions...", spinner="dots"):
