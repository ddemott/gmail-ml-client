...
def test_cli_commands():
    ...
    success, output, error = run_cli_command(["sync", "--limit", "1"])
    if "credential" in error.lower() or "auth" in error.lower():
    ...
    if "no data" in error.lower() or "insufficient" in error.lower() or success:
    ...
    if success or "no messages" in output.lower():
