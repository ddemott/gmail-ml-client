...
class TestCLICommands:
    ...
    def setup_method(self):
        ...
        self.db_patcher = patch('data_store.DATABASE_PATH', self.db_path)
    ...
    def test_ensure_labels_command(self, mock_ensure_label):
        ...
        result = self.runner.invoke(app, ['ensure-labels'])
        
        assert result.exit_code == 0
        assert "Ensured label" in result.stdout
        
        # Verify all default labels were created
        expected_labels = ["Work", "Personal", "Receipts", "Finance", "Newsletters", "Social", "Updates"]
    ...
    @patch('cli.init_db')
    def test_sync_command_success(self, mock_init_db, mock_list_messages, 
                                 mock_get_message, mock_extract_text, mock_upsert_message):
        ...
        mock_extract_text.side_effect = [
            ...
            "Second test message body"
        ]
        
        result = self.runner.invoke(app, ['sync', '--limit', '2'])
    ...
    def test_train_command(self, mock_train_from_feedback):
        ...
        result = self.runner.invoke(app, ['train', '--epochs', '3'])
        ...
        mock_train_from_feedback.assert_called_once_with(epochs=3)
    ...
    def test_predict_command_with_results(self, mock_propose):
        ...
        result = self.runner.invoke(app, ['predict', '--limit', '10'])
        ...
        mock_propose.assert_called_once_with(limit=10)
    ...
    @patch('cli.propose')
    def test_apply_command_dry_run(self, mock_propose, mock_trash_message, 
                                  mock_ensure_label, mock_modify_labels):
        ...
        result = self.runner.invoke(app, ['apply', '--dry-run'])
    ...
    @patch('cli.trash_message')
    @patch('cli.propose')
    def test_apply_command_real_run(self, mock_propose, mock_trash_message, 
                                   mock_ensure_label, mock_modify_labels):
        """Test apply command in real mode."""
        mock_propose.return_value = [
            {
                "id": "msg1",
                "action": "trash",
        ...
        mock_trash_message.return_value = {"id": "msg1"}
        
        result = self.runner.invoke(app, ['apply', '--no-dry-run'])
        
        assert result.exit_code == 0
        assert "Applied=1" in result.stdout
        assert "dry_run=False" in result.stdout
        
        # Verify actual actions were taken
        mock_trash_message.assert_called_once_with("msg1")
...
class TestCLIEndToEndWorkflows:
    ...
    @patch('cli.get_labels')
    @patch('cli.init_db')
    def test_complete_workflow(self, mock_init_db, mock_get_labels, mock_list_messages,
                              mock_get_message, mock_extract_text, mock_upsert_message,
                              mock_mark_review, mock_train_from_feedback, mock_trash_message,
                              mock_ensure_label, mock_modify_labels):
        ...
        # 1. Initialize
        result1 = self.runner.invoke(app, ['init'])
        assert result1.exit_code == 0
        assert "DB ready and Gmail auth OK" in result1.stdout
        
        # 2. Ensure labels
        result2 = self.runner.invoke(app, ['ensure-labels'])
        ...
        result3 = self.runner.invoke(app, ['sync', '--limit', '2'])
        ...
        result4 = self.runner.invoke(app, ['train', '--epochs', '2'])
        ...
        with patch('cli.propose') as mock_propose:
            ...
            result5 = self.runner.invoke(app, ['apply', '--no-dry-run'])
            assert result5.exit_code == 0
            assert "Applied=1" in result5.stdout
        ...
        mock_trash_message.assert_called()
...
class TestCLIErrorHandling:
    ...
    def test_invalid_arguments(self):
        ...
        result = self.runner.invoke(app, ['sync', '--limit', '-1'])
