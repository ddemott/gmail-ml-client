...
class DatabaseAdapter(DatabaseInterface):
    ...
    def save_prediction(self, message_id: str, prediction: PredictionResult) -> bool:
        ...
        try:
            data_store.save_prediction(message_id, {
...
class ModelAdapter(ModelInterface):
    ...
    def predict(self, text: str) -> PredictionResult:
        ...
        try:
            ...
            main_prediction = predictions[0] if predictions else {'label': 'UNKNOWN', 'confidence': 0.0}
            alternatives = [(p['label'], p['confidence']) for p in predictions[1:]] if len(predictions) > 1 else []
