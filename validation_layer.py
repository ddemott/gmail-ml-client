        ...
        if 'alternatives' in prediction_data:
            ...
            if not isinstance(alternatives, list):
                ...
                for i, alt in enumerate(alternatives):
                    ...
                    elif not (self.confidence_range[0] <= alt_confidence <= self.confidence_range[1]):
...
class TrainingDataValidator(Validator):
    ...
    def validate(self, training_data: List[Dict[str, Any]]) -> ValidationResult:
        ...
        insufficient_labels = [
            ...
            if count < self.min_samples_per_label
        ]
        ...
        result.add_info(f"Training data contains {len(training_data)} samples across {len(label_counts)} labels")
...
class ValidationRuleBuilder:
    ...
    def range_check(self, field_name: str, min_val: float, max_val: float) -> 'ValidationRuleBuilder':
            ...
            if isinstance(data, dict) and field_name in data:
                ...
                if isinstance(value, (int, float)):
                    if not (min_val <= value <= max_val):
                        result.add_error(f"Field {field_name} value {value} outside range [{min_val}, {max_val}]")
            return result
...
def validate_prediction(prediction_data: Dict[str, Any], valid_labels: Optional[Set[str]] = None) -> ValidationResult:
...
def create_prediction_validation_chain(valid_labels: Optional[Set[str]] = None) -> ValidationChain:
    ...
    return (ValidationChain()
            .add_validator("prediction_format", PredictionValidator(valid_labels)))
