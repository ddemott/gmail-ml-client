...
class TestCLICommands:
    ...
    def setup_method(self):
        ...
        self.db_patcher = patch("src.gmail_ml_client.cfg.DB_PATH", self.db_path)
    ...
    @patch("scripts.cli.get_labels")
    @patch("scripts.cli.init_db")
    ...
    @patch("scripts.cli.ensure_label")
    ...
    @patch("scripts.cli.list_messages")
    @patch("scripts.cli.get_message")
    @patch("scripts.cli.extract_text")
    @patch("scripts.cli.upsert_message")
    @patch("scripts.cli.init_db")
    ...
    @patch("scripts.cli.train_from_feedback")
    ...
    @patch("scripts.cli.modify_labels")
    @patch("scripts.cli.ensure_label")
    @patch("scripts.cli.trash_message")
    @patch("scripts.cli.propose")
...
class TestCLIEndToEndWorkflows:
    ...
    def test_complete_workflow(
        ...
        # 5. Apply actions
        with patch("scripts.cli.propose") as mock_propose:
            mock_propose.return_value = [
                {
                    "id": "msg_spam",
                    "action": "trash",
...
class TestCLIRealExecution:
    ...
    def test_individual_command_help(self):
        ...
        for cmd in commands_to_test:
            result = subprocess.run(
                [self.python_exe, "scripts/cli.py", cmd, "--help"],
                cwd="d:\\development\\Workspaces\\PythonWorkspace\\GmailClient",
                capture_output=True,
                text=True,
                env=env,
