...
def test_end_to_end_workflow():
    ...
    try:
        # Import modules
        from src.gmail_ml_client import cfg, data_store, model, sorter, trainer
        ...
        test_messages = [
            (
                "msg1",
                "Free money!",
                "Get free money now! Click here to claim your prize! Limited time offer!",
            ),
            (
                "msg2",
                "Meeting tomorrow",
                "Hi team, reminder about the meeting tomorrow at 2pm in conference room A",
            ),
            ...
            if unreviewed:
                ...
                if proposals:
                    ...
                    if all(key in proposal for key in required_keys):
                        ...
                        print(
                            f"! Proposal missing keys: {set(required_keys) - set(proposal.keys())}"
                        )
        ...
        assert False, f"End-to-end workflow failed: {e}"
    finally:
        # Cleanup
    ...
    try:
        ...
        for config in required_configs:
            if hasattr(cfg, config):
                ...
                print(
                    f"✓ {config}: {type(value).__name__} with {len(value) if hasattr(value, '__len__') else 'value'}"
                )
            else:
                print(f"✗ Missing configuration: {config}")
                assert False, f"Missing configuration: {config}"
...
def test_error_handling():
    ...
    try:
        from src.gmail_ml_client import data_store, model, preprocessor
