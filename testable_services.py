...
from interfaces import (
    GmailApiInterface, DatabaseInterface, FileSystemInterface, ModelInterface,
    TextProcessorInterface, ConfigurationInterface, LoggerInterface,
    EmailMessage, LabelInfo, PredictionResult, TrainingMetrics,
    inject_dependencies, Interfaces
)
...
class ServiceResult:
    ...
    @classmethod
    def error_result(cls, message: str, errors: List[str] = None) -> 'ServiceResult':
        """Create an error result."""
        return cls(success=False, message=message, errors=errors or [message])


class TestableGmailService:
    ...
    def initialize(self, 
                   gmail_api: GmailApiInterface = None,
                   database: DatabaseInterface = None,
                   config: ConfigurationInterface = None,
                   logger: LoggerInterface = None) -> ServiceResult:
    ...
    def sync_emails(self, query: Optional[str] = None, limit: int = 100) -> ServiceResult:
        ...
        try:
            ...
            return ServiceResult.success_result(
                data=result_data,
                message=f"Synced {stored_count} messages"
            )
...
class TestablePredictionService:
    ...
    def predict_messages(self,
                        ...
                        text_processor: TextProcessorInterface = None,
                        config: ConfigurationInterface = None,
                        logger: LoggerInterface = None) -> ServiceResult:
        ...
        try:
            ...
            if not messages:
                return ServiceResult.success_result(
                    data=[],
                    message="No unreviewed messages found"
                )
            ...
            for message in messages:
                try:
                    ...
                    if prediction.predicted_label == "SPAM" and prediction.confidence >= spam_threshold:
                        action = "trash"
                    ...
                    prediction_data = {
                        ...
                        "spam_score": prediction.confidence if prediction.predicted_label == "SPAM" else 0.0
                    }
            ...
            logger.info(f"Prediction complete: {len(predictions)} predictions, {failed_count} failed")
            return ServiceResult.success_result(
                data=result_data,
...
class TestableTrainingService:
    ...
    def train_model(self,
        ...
        try:
            ...
            return ServiceResult.success_result(
                data=result_data,
                message=f"Model trained with {len(training_pairs)} samples"
            )
...
class TestableActionService:
    ...
    def apply_actions(self,
        ...
        try:
                        ...
                        if confidence >= spam_threshold:
                            ...
                            if not dry_run:
                                ...
                                if success:
                                    ...
                                    failed_count += 1
                    elif any(label in ["Work", "Personal", "Finance"] for label in message.labels):
                        confidence = 0.95
                        if confidence >= certain_threshold:
                            target_label = message.labels[0] if message.labels else "Personal"
            ...
            logger.info(f"Actions {action_word}: {len(applied_actions)} total, {failed_count} failed")
...
def create_gmail_service() -> TestableGmailService:
    """Create Gmail service with injected dependencies."""
    from interfaces import get_dependency, Interfaces
    
    return TestableGmailService(
...
def create_prediction_service() -> TestablePredictionService:
    """Create prediction service with injected dependencies."""
    from interfaces import get_dependency, Interfaces
    
    return TestablePredictionService(
        database=get_dependency(Interfaces.DATABASE),
...
def create_training_service() -> TestableTrainingService:
    """Create training service with injected dependencies."""
    from interfaces import get_dependency, Interfaces
    
    return TestableTrainingService(
        database=get_dependency(Interfaces.DATABASE),
        model=get_dependency(Interfaces.MODEL),
        text_processor=get_dependency(Interfaces.TEXT_PROCESSOR),
...
def create_action_service() -> TestableActionService:
    """Create action service with injected dependencies."""
    from interfaces import get_dependency, Interfaces
    
    return TestableActionService(
        gmail_api=get_dependency(Interfaces.GMAIL_API),
        database=get_dependency(Interfaces.DATABASE),
        config=get_dependency(Interfaces.CONFIGURATION),
        logger=get_dependency(Interfaces.LOGGER)
