...
def analyze_classification_errors():
    ...
    try:
        ...
        total_emails = session.query(Message).filter(
        ...
        if inconsistencies:
            ...
            for i, error in enumerate(inconsistencies[:10], 1):  # Show top 10
                ...
                print(f"   Model now predicts: {error['predicted']} (conf: {error['confidence']:.2f})")
                ...
                if error['stored'] == 'SPAM' and error['predicted'] != 'SPAM':
                    print(f"   🚨 Pattern: Potential false positive (legitimate email marked as spam)")
